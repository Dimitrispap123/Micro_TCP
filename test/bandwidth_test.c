...
server_microtcp (uint16_t listen_port, const char *file)
{
    ...
    if (microtcp_accept(&socket, (struct sockaddr*)&address, sizeof(address)) == -1) {
    ...
    bytes_received=microtcp_recv(&socket, buffer, MICROTCP_RECVBUF_LEN, 0);
    clock_gettime(CLOCK_MONOTONIC_RAW, &end_time);
	  fwrite(buffer, sizeof(char), strlen(buffer), fp);
    // Print statistics
    printf("Bytes Received: %ld\n", bytes_received);
    print_statistics(bytes_received, start_time, end_time);
...
int client_microtcp (const char *serverip, uint16_t server_port, const char *file){
    ...
    // Connect to server
    if (microtcp_connect(&socket, (struct sockaddr*)&address, sizeof(address)) == -1) {
    ...
    // Send data to server
    if (microtcp_send(&socket, buffer, fileSize, 0) == -1) {
